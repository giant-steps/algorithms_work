...
while count < span:
    ...
    while back >= 0:
        ...
        if back == count:
            if suffixes[back] not in substrings and suffixes[back] != '':       ## find better way to do this
                substrings.append(suffixes[back])

        else:
            if "".join(suffixes[count].rsplit(suffixes[back])) not in substrings and "".join(suffixes[count].rsplit(suffixes[back])) != '':
